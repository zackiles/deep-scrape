...
/**
 * Sets up a PhantomJS instance and configures all the callbacks.
 * Returns once PhantomJS is open with a new tab instance.
 *
 * @method setupPage
...
Browser.prototype.setupPage = function(page){
  ...
  return new Promise(function (resolve) {
    ...
    page.get('settings.userAgent', function(userAgent){
      userAgent = [userAgent,npmPackage.name,npmPackage.repository.url].join(' ');
...
Browser.prototype.openPage = function(pageOptions){
  ...
  return new Promise(function (resolve, reject) {
    self.open().then(function(){
      ...
      logger.trace('METHOD:', self.page.method, '@ URL:', self.page.url, 'SANITIZED:', self.page.urlSanitized);
        ...
        self.pageInstance.open(self.page.urlSanitized, function (status) {
          ...
          utilities.waitForPageToLoad(self.pageInstance, function(){
            self.pageInstance.get('content',function(content){
              ...
              .then(function(results){
                logger.trace('Page ' + (self.page.redirectURL || self.page.url) + ' rendered successfully');
                resolve(self.page);
              }).catch(reject);
            });
          });
...
Browser.prototype.handleResourceRequested = function(request){
  ...
  // don't add aborted requests to the page cache.
  if(request.aborted) {
    self.page.blockedResources.push(request.url);
    return;
  }

  if(self.page._isLoaded === true) return;

  // save the first request header for the page if the request matches the page url
  if(utilities.compareUrl(self.page.urlSanitized, request.url)){
    self.page.reqHeaders = request.headers;
  }else{
    // if it's not the main page url then it's a resource
...
Browser.prototype.handleResourceReceived = function(response){
  ...
  if(response.id === 1 || !self.page._resources){
    // if this is the first respose then it is for the main page
    // so we'll save some of the response message
    ...
    // this is not the main page, it's a resource
...
/**
 * Callback fired when a resource of the page times out.
 *
...
/**
 * Callback fired when an error has thrown when requesting a resource for the page
 *
 * @method handleResourceTimeout
 * @return undefined
 */
Browser.prototype.handleResourceError = function(resourceError){
  if(this.page._isLoaded === true) return;
  // add the url to the blocked resources list, it'll only last this browser instance
  this.browserOptions.blockedResources.push(resourceError.url);
  ...
  return new Promise(function (resolve, reject) {
    ...
    cache.get(cacheKey).then(function(results){
      ...
      restler.get(resource.url, {
        ...
        cache.set(cacheKey, resource).then(function(){
          resolve(resource);
        });
      });
    });
...
Browser.prototype.fetchMultiResource = function (list) {
  return Promise.each(list, _.bind(this.fetchResource, this));
};

/**
 * Finishes up page processing, caches all fetched resources
 * for further requests and processing.
 *
 * @method finalize
 * @return undefined
 */
Browser.prototype.finalize = function () {
  var self = this;
  if(this.page._isLoaded === true) return;
  logger.trace('Done rendering', self.page.url);
  logger.trace('Fetching resources for caching');
  return new Promise(function (resolve, reject) {
    return Promise.each([
      self.page.resources.js,
      self.page.resources.json,
      self.page.resources.xml,
      self.page.resources.html
    ], _.bind(self.fetchMultiResource, self))
    .then(function(){
      logger.trace('Resources fetched successfully');
      self.page._isLoaded = true;
      self.pages.push(self.page);
