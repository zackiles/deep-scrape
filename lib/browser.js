    ...
    self.open().then(function(){
      ...
      cache.get(self.page.id).then(function(results){
        if(results) {
          ...
          return resolve(self.page.toObject());
        }
...
Browser.prototype.getBlockedResources = function(){
  ...
  return self.page.blockedResources || [];
};
...
Browser.prototype.finalize = function (url) {
  ...
  return fetchResourceList(self.page.resources.js)
  .then(function(){
    return fetchResourceList(self.page.resources.json);
  })
  .then(function(){
    return fetchResourceList(self.page.resources.xml);
  })
  .then(function(){
    return fetchResourceList(self.page.resources.html);
  })
  .then(function(){
    self.page._isLoaded = true;
    self.pages.push(self.page);
...
function fetchResourceList(list){
  return Promise.all(list.map(fetchResource)).spread(function() {
