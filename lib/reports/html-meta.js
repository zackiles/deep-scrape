...
HtmlMetaReport.prototype._parsePage = function(page){
  var self = this;
  if(!page || !page.url) throw new Error('No page was provided to add');
  ...
  this.feeds = _.uniq(this.feeds);
  this.internalLinks = _.uniq(this.internalLinks);
  this.externalLinks = _.uniq(this.externalLinks);
...
function getTitle($){
  var t = utilities.humanize($('meta[property="og:title"]').attr('content'));
  if(t) return t;
  utilities.humanize($('meta[property="twitter:title"]').attr('content'));
  if(t) return t;
  utilities.humanize($('title').text());
  if(t) return t;
...
function getImage($, baseUrl){
  ...
  i = $('meta[property="twitter:image"]').attr('content');
  if(i){
    link = utilities.canonicalizeUrl(baseUrl, i);
    return link ? link : i;
  }
