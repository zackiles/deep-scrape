      ...
      cache.get(self.page.id).then(function(results){
        ...
        self.pageInstance.open(self.page.urlSanitized, function (status) {
          ...
          utilities.waitForPageToLoad(self.pageInstance, function(){
            fetchResourceRaw({url:self.page.redirectURL || self.page.urlSanitized}).then(function(results){
              self.page.document = results;
              ...
              self.pageInstance.get('content',function(content){
                ...
                cache.set(self.page.id, self.page.toObject())
                  .then(function(){
                    self.page._isLoaded = true;
                    resolve(self.page);
                  })
...
function fetchResourceRaw(req){
  return new Promise(function (resolve, reject) {
    restler.get(req.url).on('complete', function(results) {
