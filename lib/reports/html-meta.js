...
HtmlParser.prototype.report = function(page){
  var self = this;
  ...
  this.charset = $('meta[charset]').attr('charset');
  this.size = parseFloat((Buffer.byteLength(page.documentRendered, 'utf8') / 1024).toFixed(2));
  this.title = getTitle($);
...
function getDescription($){
 ...
 // if(d) return utilities.humanize(d);

 var d = utilities.humanize($('meta[property="og:description"]').attr('content'));
  if(d) return d;
  return '';
}
